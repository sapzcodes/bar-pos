...
export default async function handler(req, res) {
  ...
  if (!GAS_URL) {
    return res.status(500).json({ success: false, message: "GAS_URL not configured" });
  }
  ...
  // Allowlist of permitted actions — blocks any rogue calls
  const ALLOWED_ACTIONS = [
    "getInventory", "getOrders", "getBills", "getSettings",
    "verifyPin", "addInventoryItem", "updateInventoryItem",
    "deleteInventoryItem", "adjustStock", "saveOrder",
    "updateOrderStatus", "saveBill", "sendResetPin", "saveSettings",
  ];
